...
export default function UsagePage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white font-[family-name:var(--font-display)]">Usage Analytics</h1>
        <p className="text-slate-400 mt-1">Monitor credit consumption and API usage patterns.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Card>
          ...
          <p className="text-3xl font-bold text-white font-[family-name:var(--font-mono)] mt-2 tabular-nums">965</p>
        ...
        <Card>
          ...
          <p className="text-3xl font-bold text-white font-[family-name:var(--font-mono)] mt-2 tabular-nums">152</p>
        </Card>
        <Card>
          <p className="text-xs text-slate-500 font-[family-name:var(--font-display)]">Avg credits/day</p>
          <p className="text-3xl font-bold text-white font-[family-name:var(--font-mono)] mt-2 tabular-nums">32.2</p>
        </Card>
      </div>
            ...
            {USAGE_BY_ENDPOINT.map((ep) => (
              <div key={ep.endpoint}>
                <div className="flex items-center justify-between mb-1.5">
                  <code className="text-xs text-slate-400 font-[family-name:var(--font-mono)]">{ep.endpoint}</code>
                  <span className="text-xs text-slate-500 font-[family-name:var(--font-mono)] tabular-nums">{ep.credits} credits</span>
                </div>
          ...
          <div className="mt-4 flex items-end gap-2 h-48">
            {USAGE_BY_DAY.map((day) => (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs text-slate-500 font-[family-name:var(--font-mono)] tabular-nums">{day.credits}</span>
